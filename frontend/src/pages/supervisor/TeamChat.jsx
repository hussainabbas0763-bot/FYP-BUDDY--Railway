...
export default function TeamChat() {
  ...
  const reorderRoomsWithSaved = useCallback((rooms) => {
    try {
      const key = `chat_order_supervisor_${user?._id || 'anon'}`;
  ...
  useEffect(() => {
    ...
    const fetchRooms = async () => {
      ...
      try {
        ...
        if (ignore) return;
        const fetchedRooms = (res.data?.rooms || []).filter(room => room.type !== 'public');
  ...
  useEffect(() => {
    ...
    socket.on("chat:rooms", (serverRooms) => {
      ...
      // Filter out public chats for supervisor
      serverRooms = serverRooms.filter(room => room.type !== 'public');
  ...
  const saveScrollPosition = () => {
    if (messagesContainerRef.current && activeChatId) {
      ...
      localStorage.setItem(`chat_scroll_supervisor_${activeChatId}`, JSON.stringify(scrollData));
    }
  };

  const restoreScrollPosition = () => {
    if (messagesContainerRef.current && activeChatId) {
      const savedData = localStorage.getItem(`chat_scroll_supervisor_${activeChatId}`);
      if (savedData) {
        try {
  ...
  const endVideoCall = () => {
    try {
      ...
      // Send call notification with duration - only initiator posts summary
  ...
  const getParticipantsMap = () => {
    ...
    // Get participants from activeChat
    const participants = activeChat.participants || activeChat.members || [];
  ...
  return (<>
    <SEO 
      title="Team Chat | FYP Buddy - Communicate with Groups"
      description="Chat with your supervised FYP groups. Provide guidance, answer questions, and collaborate with student teams in real-time."
      keywords="supervisor chat, team communication, FYP guidance, student collaboration"
    />
